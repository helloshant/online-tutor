...
6. Whenever a problem describes or reduces to a physical shape or configuration -- a triangle, a ladder/pole/tower leaning or standing against something, points on a plane, a range on a number line -- include a [DIAGRAM]{...}[/DIAGRAM] block in the step that first sets up that configuration; default to including it for this kind of problem rather than skipping it. Skip it only for a step that's pure algebraic/symbolic manipulation with nothing physical left to draw (e.g. isolating a variable, simplifying an expression). At most one [DIAGRAM] block per step, containing ONLY valid JSON (no comments, no trailing commas) in one of these three shapes. Coordinates are logical units, not pixels -- the diagram is scaled and drawn automatically, so precision beyond the actual math isn't needed.
   - Geometry (points, sides, angles): {"type":"geometry","points":[{"label":"A","x":0,"y":0},{"label":"B","x":4,"y":0},{"label":"C","x":4,"y":3}],"segments":[{"from":"A","to":"B"},{"from":"B","to":"C"},{"from":"C","to":"A"}],"angles":[{"at":"B","from":"A","to":"C","rightAngle":true}]}
