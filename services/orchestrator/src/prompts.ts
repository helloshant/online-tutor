...
6. Whenever a problem describes or reduces to a physical shape or configuration -- a triangle, a ladder/pole/tower leaning or standing against something, an angle of elevation/depression, points on a plane, a range on a number line -- include a [DIAGRAM]{...}[/DIAGRAM] block in the step that first sets up that configuration; default to including it for this kind of problem rather than skipping it, even one involving two triangles or an angle measured from an implied horizontal/vertical line rather than from a side that's already drawn -- add an extra point purely to mark that reference line if the problem itself doesn't name one (see the second geometry example below). Skip it only for a step that's pure algebraic/symbolic manipulation with nothing physical left to draw (e.g. isolating a variable, simplifying an expression). At most one [DIAGRAM] block per step, containing ONLY valid JSON (no comments, no trailing commas) in one of these three shapes. Coordinates are logical units, not pixels -- the diagram is scaled and drawn automatically, so precision beyond the actual math isn't needed.
   ...
   - Geometry with an angle of elevation/depression -- "O" is the horizontal-line and "H" is a helper point placed purely to mark that horizontal (it's never mentioned by the problem itself, only added so the angle has something to be measured against): {"type":"geometry","points":[{"label":"O","x":0,"y":0},{"label":"H","x":4,"y":0},{"label":"T","x":4,"y":3}],"segments":[{"from":"O","to":"H"},{"from":"O","to":"T"},{"from":"H","to":"T","label":"height"}],"angles":[{"at":"O","from":"H","to":"T","label":"30°"}],"title":"Angle of elevation to the tower top"} -- for two triangles sharing one vertical (e.g. two elevation/depression angles from the same building to a tower's top and bottom), use the same pattern twice from the same horizontal-reference point, with all the shared points (the building, the tower's top and bottom) in one single [DIAGRAM] block rather than two separate ones.
