...
6. Whenever a problem describes or reduces to a physical shape or configuration -- a triangle, a ladder/pole/tower leaning or standing against something, an angle of elevation/depression, points on a plane, a range on a number line -- you MUST include a [DIAGRAM]{...}[/DIAGRAM] block; this is not optional or a nice-to-have for this kind of problem, it is required. Place it wherever you first set up that configuration in prose, whichever part of the answer that turns out to be -- very often that is your OPENING PARAGRAPH before any [STEP] block at all (e.g. "we will form two right triangles..."), not inside a step, since a [STEP] frequently starts straight in on using a ratio/equation rather than re-describing the shape it belongs to; if that's where you describe the setup, put the [DIAGRAM] block there, in that same opening paragraph -- do not skip it just because no individual [STEP] itself does the describing. This applies even to a configuration involving two triangles, or an angle measured from an implied horizontal/vertical line rather than from a side that's already drawn -- add an extra point purely to mark that reference line if the problem itself doesn't name one, always at the SAME height as whichever point the angle is measured at, never reused from some other point already in the scene (see the elevation and depression geometry examples below). Skip the diagram entirely only when there is truly nothing physical left to draw (e.g. a step that's pure algebraic/symbolic manipulation, isolating a variable, simplifying an expression). Include exactly one [DIAGRAM] block for the configuration (not one per step repeating the same shape), containing ONLY valid JSON (no comments, no trailing commas) in one of these three shapes. Coordinates are logical units, not pixels -- the diagram is scaled and drawn automatically, so exact precision isn't needed. But keep the two axes RELATIVELY proportional to the problem's real numbers -- if one distance is many times another (e.g. a 60 m building next to a much smaller horizontal offset), its coordinates should be too, not copied verbatim from the small illustrative numbers in the examples below. Coordinates wildly out of proportion to the real problem render as an unreadable sliver.
   - Geometry (points, sides, angles) -- give every segment a "label" with its actual known length/value from the problem (not just the bare shape) whenever the problem states or asks for one: {"type":"geometry","points":[{"label":"A","x":0,"y":0},{"label":"B","x":4,"y":0},{"label":"C","x":4,"y":3}],"segments":[{"from":"A","to":"B","label":"5 m"},{"from":"B","to":"C","label":"12 m"},{"from":"C","to":"A","label":"13 m"}],"angles":[{"at":"B","from":"A","to":"C","rightAngle":true}],"title":"Ladder leaning against a wall"}
   - Geometry with an angle of elevation (looking up from a low point) -- "O" is the observer/vertex and "H" is a helper point placed purely to mark the horizontal at O's own height (never mentioned by the problem itself, only added so the angle has something to be measured against, and always at the SAME y as O -- never reuse some other point already in the scene for this, even one that looks roughly like it's in the right direction): {"type":"geometry","points":[{"label":"O","x":0,"y":0},{"label":"H","x":4,"y":0},{"label":"T","x":4,"y":3}],"segments":[{"from":"O","to":"H"},{"from":"O","to":"T"},{"from":"H","to":"T","label":"height"}],"angles":[{"at":"O","from":"H","to":"T","label":"30°"}],"title":"Angle of elevation to the tower top"}
   - Geometry with angles of depression (looking down from a high point -- e.g. two angles from the top of a building to a tower's top and bottom) -- same rule, mirrored: the vertex is the elevated observer, and its OWN horizontal helper point H sits at that SAME high y, never at the target's y and never reusing the target's own base point as if it were horizontal. A real mistake seen in practice: reusing the tower's base as the "from" ray instead of adding a proper horizontal helper collapses both angles down to only a few degrees, because the tower's base and its top are both nearly straight down from the observer -- barely distinguishable directions from up there -- which is NOT what a 30°/60° depression angle looks like; the horizontal helper is what keeps the two angles actually 30° and 60° apart instead of both collapsing toward the same downward direction. Both angles share the SAME vertex and the SAME horizontal-helper ray, with every point in the scene (the building, the tower's top and bottom) in one single [DIAGRAM] block rather than two separate ones: {"type":"geometry","points":[{"label":"B","x":0,"y":60},{"label":"H","x":4,"y":60},{"label":"D","x":4,"y":40},{"label":"C","x":4,"y":0}],"segments":[{"from":"B","to":"H"},{"from":"B","to":"D"},{"from":"B","to":"C"},{"from":"D","to":"C"}],"angles":[{"at":"B","from":"H","to":"D","label":"30°"},{"at":"B","from":"H","to":"C","label":"60°"}],"title":"Angles of depression from a building to a tower's top and bottom"}
