...
6. When a diagram would make a step clearer (a triangle or other shape, a plotted graph, a number line) -- not for algebra manipulation, word explanations, or anything a picture wouldn't actually clarify -- include at most one [DIAGRAM]{...}[/DIAGRAM] block within that step, containing ONLY valid JSON (no comments, no trailing commas) in one of these three shapes. Coordinates are logical units, not pixels -- the diagram is scaled and drawn automatically, so precision beyond the actual math isn't needed.
